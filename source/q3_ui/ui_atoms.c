...
void UI_SetActiveMenu(uiMenuCommand_t menu)
{
	...
	case UIMENU_MAIN:
	{
		int gt = trap_Cvar_VariableValue("g_gametype");
		if (!gt)
	...
	case UIMENU_NEED_CD:
		UI_ConfirmMenu("Insert the CD", (voidfunc_f)NULL, NeedCDAction, qfalse);
		return;
	case UIMENU_BAD_CD_KEY:
		UI_ConfirmMenu("Bad CD Key", (voidfunc_f)NULL, NeedCDKeyAction, qfalse);
		return;
	case UIMENU_INGAME:
		/*
		//GRank
...
void Inv_UI_ErrorBox(void)
{
	...
	while (1)
	{
		...
		if (strlen(Final) + strlen(Text) >= sizeof(ErrorText))
			break;

		if (i != 1)
			Final = va("%s %s", Final, Text);
		else
			Final = va("%s", Text);
		++i;
	}

	strcpy(ErrorText, Final);

	UI_ConfirmMenu(ErrorText, NULL, NULL, qtrue);
}


int Inv_atol(char *str)
{
	int i, n = 0, mul = 1,
		l = strlen(str);
...
qboolean UI_ConsoleCommand(int realTime)
{
	...
	if (Q_stricmp (cmd, "Inv_ui_MarineSelection") == 0)
	{
		trap_Cvar_Set("inv_IGM", "1");
		Inv_UI_MarineSelection_f();
		return qtrue;
	}

	if (Q_stricmp (cmd, "Inv_ui_AlienSelection") == 0)
	{
		trap_Cvar_Set("inv_IGM", "1");
		Inv_UI_AlienSelection_f();
		return qtrue;
	}

	if (Q_stricmp (cmd, "Inv_ui_TeamSelection") == 0)
	{
		int NbRed = Inv_atol(UI_Argv(1)),
			NbBlue = Inv_atol(UI_Argv(2));

		trap_Cvar_Set("inv_IGM", "1");
		UI_TeamMainMenu(NbRed, NbBlue);
		return qtrue;
	}

	if (Q_stricmp (cmd, "Inv_ui_Error") == 0)
	{
		Inv_UI_ErrorBox();
		return qtrue;
	}
