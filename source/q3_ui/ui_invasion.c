...
void MarineWeapon_Init(void)
{
	...
	y += INV_MENU_VERTICAL_SPACING;
	InitMenuText(&s_MarineWeapon->Sniper, MTYPE_PTEXT, QMF_LEFT_JUSTIFY | QMF_PULSEIFFOCUS,
					x, y, e_Id_Sniper, Marine_Event, "5 - Sniper Rifle",
					color_red, UI_LEFT | UI_SMALLFONT | UI_DROPSHADOW);

	InitDefault(x, y, 6, Marine_Event);
	...
	Menu_AddItem(&s_MarineWeapon->menu, &s_MarineWeapon->Sniper);
...
{
	InvMenuStartTime = -1;//uis.realtime;
	MarineWeapon_Init();
}
