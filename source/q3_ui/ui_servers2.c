...
#define GAMES_CTF				4
#define GAMES_INVASION		5
#define GAMES_DESTROY		6*/

#define GAMES_INVASION		1
#define GAMES_DESTROY		2
#define GAMES_CTF				3
...
static char* gamenames[] =
{
	...
	"Invasion",// b"INVASION_VERSION,	// Invasion
...
static void ArenaServers_UpdateMenu(void)
{
	...
	for (i = 0, j = 0; i < count; i++, servernodeptr++)
	{
		...
		case GAMES_INVASION:
			if (servernodeptr->gametype != GT_INVASION)
				continue;

			if (Q_stricmp(servernodeptr->gamename, "invasion"))
				continue;

			break;

		case GAMES_DESTROY:
			if (servernodeptr->gametype != GT_DESTROY)
				continue;
	...
	if (i <= g_arenaservers.numfavoriteaddresses-1)
	{
		if (i < g_arenaservers.numfavoriteaddresses-1)
		{
			...
			memcpy(&g_arenaservers.favoriteaddresses[i],
					...
					(g_arenaservers.numfavoriteaddresses - i - 1) * sizeof(g_arenaservers.favoriteaddresses[0]));
					// Too : lazy bug... (g_arenaservers.numfavoriteaddresses - i - 1)*sizeof(MAX_ADDRESSLENGTH));
		}
...
static void ArenaServers_MenuInit(void)
{
	...
	g_gametype = Com_Clamp(0, 3, ui_browserGameType.integer);
