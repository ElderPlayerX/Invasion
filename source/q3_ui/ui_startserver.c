...
//static int gametype_remap[] = {GT_FFA, GT_TEAM, GT_TOURNAMENT, GT_CTF, GT_INVASION, GT_DESTROY };
//static int gametype_remap2[] = {0, 2, 0, 1, 3, 4, 5};
static int gametype_remap[] = { GT_INVASION, GT_DESTROY, GT_CTF };
static int gametype_remap2[] = {0, 0, 0, 0, 2, 0, 1};
...
static int GametypeBits(char *string)
{
	...
	while(1)
	{
		...
		if (Q_stricmp(token, "tte") == 0)
		{
			bits |= (1 << GT_INVASION) | (1 << GT_CTF);
			continue;
		}

		if (Q_stricmp(token, "inv") == 0)
		{
			bits |= (1 << GT_DESTROY) | (1 << GT_INVASION);
			continue;
		}

		if (Q_stricmp(token, "dae") == 0)
		{
			bits |= (1 << GT_DESTROY) | (1 << GT_INVASION);
			continue;
		}
...
/*
=================
Too: ServerOptions_StatusBarRoundTime
=================
*/
static void ServerOptions_StatusBarRoundTime( void* ptr) {
	UI_DrawString( 320, 440, "Time of rounds in seconds", UI_CENTER|UI_SMALLFONT, colorWhite);
}


/*
=================
Too: ServerOptions_StatusBarSwapPeriod
=================
*/
static void ServerOptions_StatusBarSwapPeriod( void* ptr) {
	UI_DrawString( 320, 440, "Number of rounds before teams swap (0 = no swap)", UI_CENTER|UI_SMALLFONT, colorWhite);
}
...
typedef struct
{
	...
	menufield_s			RoundTime;
	menufield_s			SwapPeriod;
...
static void ServerOptions_Start(void)
{
	...
	RoundTime = atoi(s_serveroptions.RoundTime.field.buffer);
	SwapPeriod = atoi(s_serveroptions.SwapPeriod.field.buffer);
	...
	case GT_TEAM:
		...
		trap_Cvar_SetValue("ui_team_friendly", friendlyfire); //Too : a bug was here
		break;

	case GT_DESTROY:
	case GT_INVASION:
		trap_Cvar_SetValue("ui_inv_winlimit", fraglimit);
		trap_Cvar_SetValue("ui_inv_timelimit", timelimit);
		trap_Cvar_SetValue("ui_inv_friendly", friendlyfire);
		trap_Cvar_SetValue("ui_Inv_RoundTime", RoundTime);
		trap_Cvar_SetValue("ui_Inv_SwapPeriod", SwapPeriod);
		break;

	case GT_CTF:
		trap_Cvar_SetValue("ui_ctf_capturelimit", fraglimit); //Too : a bug was here
		trap_Cvar_SetValue("ui_ctf_timelimit", timelimit);
		trap_Cvar_SetValue("ui_ctf_friendly", friendlyfire); //Too : a bug was here
		break;
	}
	...
	trap_Cvar_SetValue ("Inv_RoundTime", Com_Clamp(10, RoundTime, RoundTime));
	trap_Cvar_SetValue ("Inv_SwapPeriod", Com_Clamp(0, 20, SwapPeriod));
...
static void ServerOptions_SetMenuItems(void)
{
	...
	case GT_DESTROY:
	case GT_INVASION:
		Com_sprintf(s_serveroptions.flaglimit.field.buffer, 4, "%i", (int)Com_Clamp(0, 100, trap_Cvar_VariableValue("ui_inv_winlimit")));
		Com_sprintf(s_serveroptions.timelimit.field.buffer, 4, "%i", (int)Com_Clamp(0, 999, trap_Cvar_VariableValue("ui_inv_timelimit")));
		Com_sprintf(s_serveroptions.RoundTime.field.buffer, 4, "%i", (int)Com_Clamp(10, 900, trap_Cvar_VariableValue("ui_inv_RoundTime")));
		Com_sprintf(s_serveroptions.SwapPeriod.field.buffer, 4, "%i", (int)Com_Clamp(0, 20, trap_Cvar_VariableValue("ui_inv_SwapPeriod")));
		s_serveroptions.friendlyfire.curvalue = (int)Com_Clamp(0, 1, trap_Cvar_VariableValue("ui_inv_friendly"));
		break;
...
static void ServerOptions_MenuInit(qboolean multiplayer)
{
	...
	s_serveroptions.gametype = (int)Com_Clamp(3, 6, trap_Cvar_VariableValue("g_gameType"));
	...
	if (s_serveroptions.gametype != GT_CTF
		&& s_serveroptions.gametype != GT_INVASION
		&& s_serveroptions.gametype != GT_DESTROY)
	{
	...
	else
	{
		...
		if (s_serveroptions.gametype == GT_INVASION
			|| s_serveroptions.gametype == GT_DESTROY)
			s_serveroptions.flaglimit.generic.name       = "Win Limit:";
		else
	...
	{
		y += BIGCHAR_HEIGHT+2;
		s_serveroptions.RoundTime.generic.type			= MTYPE_FIELD;
		s_serveroptions.RoundTime.generic.name			= "Round Time:";
		s_serveroptions.RoundTime.generic.flags		= QMF_NUMBERSONLY|QMF_PULSEIFFOCUS|QMF_SMALLFONT;
		s_serveroptions.RoundTime.generic.x				= OPTIONS_X;
		s_serveroptions.RoundTime.generic.y				= y;
		s_serveroptions.RoundTime.generic.statusbar	= ServerOptions_StatusBarRoundTime;
		s_serveroptions.RoundTime.field.widthInChars	= 3;
		s_serveroptions.RoundTime.field.maxchars		= 3;

		y += BIGCHAR_HEIGHT+2;
		s_serveroptions.SwapPeriod.generic.type			= MTYPE_FIELD;
		s_serveroptions.SwapPeriod.generic.name			= "Swap Period:";
		s_serveroptions.SwapPeriod.generic.flags			= QMF_NUMBERSONLY|QMF_PULSEIFFOCUS|QMF_SMALLFONT;
		s_serveroptions.SwapPeriod.generic.x				= OPTIONS_X;
		s_serveroptions.SwapPeriod.generic.y				= y;
		s_serveroptions.SwapPeriod.generic.statusbar		= ServerOptions_StatusBarSwapPeriod;
		s_serveroptions.SwapPeriod.field.widthInChars	= 2;
		s_serveroptions.SwapPeriod.field.maxchars			= 2;
	}
	...
	if (s_serveroptions.gametype == GT_INVASION
		|| s_serveroptions.gametype == GT_DESTROY)
	{
		Menu_AddItem(&s_serveroptions.menu, &s_serveroptions.RoundTime);
		Menu_AddItem(&s_serveroptions.menu, &s_serveroptions.SwapPeriod);
	}
	if (s_serveroptions.gametype >= GT_TEAM)
	{
