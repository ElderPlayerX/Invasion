...
static void UI_SPPostgameMenu_DrawAwardsPresentation(int timer)
{
	...
	if (!postgameMenuInfo.playedSound[awardNum])
	{
		...
		trap_S_StartLocalSound(trap_S_RegisterSound(ui_medalSounds[postgameMenuInfo.awardsEarned[awardNum]]), CHAN_ANNOUNCER);
	}
}
...
void UI_SPPostgameMenu_Cache(void)
{
	...
	if (buildscript)
	{
		trap_S_RegisterSound("music music/loss.wav");
		trap_S_RegisterSound("music music/win.wav");
...
void UI_SPPostgameMenu_f(void)
{
	...
	if (playerGameRank != 1)
	{
		postgameMenuInfo.winnerSound = trap_S_RegisterSound(va("sound/player/announce/%s_wins.wav", postgameMenuInfo.placeNames[0]));
		trap_Cmd_ExecuteText(EXEC_APPEND, "music music/loss\n");
	}
	else
	{
		postgameMenuInfo.winnerSound = trap_S_RegisterSound("sound/player/announce/youwin.wav");
