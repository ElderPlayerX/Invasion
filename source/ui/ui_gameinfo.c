...
void UI_LoadArenas(void)
{
	...
	for (n = 0; n < ui_numArenas; n++)
	{
		...
		str = Info_ValueForKey(ui_arenaInfos[n], "longname");
		if (!Q_stricmp(str, ""))
			str = Info_ValueForKey(ui_arenaInfos[n], "map");
		uiInfo.mapList[uiInfo.mapCount].mapName = String_Alloc(str);
		...
		if (*type)
		{
			...
			if (strstr(type, "inv"))
			{
				uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_INVASION);
				uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_DESTROY);
			}
			if (strstr(type, "tte"))
			{
				uiInfo.mapList[uiInfo.mapCount].typeBits |= (1 << GT_INVASION);
