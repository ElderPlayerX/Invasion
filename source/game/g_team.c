...
void QDECL PrintMsg(gentity_t *ent, const char *fmt, ...)
{
	...
	Inv_SendServerCommand(((ent == NULL) ? -1 : ent-g_entities), va("print \"%s\"", msg));
}
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker)
{
	...
	if (targ->client->ps.powerups[enemy_flag_pw])
	{
		...
		if (g_gametype.integer == GT_INVASION)
			AddScore(attacker, targ->r.currentOrigin, INV_FRAG_CARRIER_BONUS);
		else
			AddScore(attacker, targ->r.currentOrigin, CTF_FRAG_CARRIER_BONUS);

		attacker->client->pers.teamState.fragcarrier++;
		if (g_gametype.integer == GT_INVASION)
			PrintMsg(NULL, "%s" S_COLOR_WHITE " fragged Alien Egg carrier!\n",
	...
	if (targ->client->pers.teamState.lasthurtcarrier &&
		...
		if (g_gametype.integer == GT_INVASION)
			AddScore(attacker, targ->r.currentOrigin, INV_CARRIER_DANGER_PROTECT_BONUS);
		else
	...
	{
	case TEAM_RED:
		c = AlienGoalClassName ? AlienGoalClassName: "team_CTF_redflag";
		break;
	case TEAM_BLUE:
		c = MarineGoalClassName ? MarineGoalClassName: "team_CTF_blueflag";
		break;
	default:
		return;
	}
	...
	if (((VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS &&
		...
		if (g_gametype.integer == GT_INVASION)
			AddScore(attacker, targ->r.currentOrigin, INV_FLAG_DEFENSE_BONUS);
		else
	...
	if (carrier && carrier != attacker)
	{
		...
		{
			if (g_gametype.integer == GT_INVASION)
				AddScore(attacker, targ->r.currentOrigin, INV_CARRIER_PROTECT_BONUS);
			else
...
int Team_TouchOurFlag(gentity_t *ent, gentity_t *other, int team)
{
	...
	{
#endif
	if (MarineGoalPickupName)
		PrintMsg(NULL, "%s" S_COLOR_WHITE " teleported the %s!\n",
					cl->pers.netname, MarineGoalPickupName);
	else
	...
	if (level.warmupTime || level.Period == e_Period_WaitRestart)
			//Too: no scoring during pre-match warmup & before restart
	{
		Team_ResetFlags();
		return 0;
	}
	...
	// other gets another x frag bonus
	if (g_gametype.integer == GT_INVASION)
	{
		InvasionWin(level.MarineTeam, qfalse);

		AddScore(other, ent->r.currentOrigin, INV_CAPTURE_BONUS);
	}
	else
	{
...
int Team_TouchEnemyFlag(gentity_t *ent, gentity_t *other, int team)
{
...
#endif
		if (MarineGoalPickupName)
			PrintMsg (NULL, "%s" S_COLOR_WHITE " got the %s!\n",
						other->client->pers.netname, MarineGoalPickupName);
		else
	...
	if (g_gametype.integer == GT_INVASION)
	{
		if (ent->s.modelindex2 == 0)						//Too: ent->s.modelindex2 is non-zero on items if they are dropped
			AddScore(other, ent->r.currentOrigin, INV_FIRST_FLAG_BONUS);
		else
			AddScore(other, ent->r.currentOrigin, INV_FLAG_BONUS);
	}
	else
...
int Pickup_Team(gentity_t *ent, gentity_t *other)
{
	...
	if (!strcmp(ent->classname, "team_CTF_redflag") || !strcmp(ent->classname, "team_CTF_teleporter"))
	{
		team = TEAM_RED;
	}
	else if (!strcmp(ent->classname, "team_CTF_blueflag") || !strcmp(ent->classname, "team_CTF_alienegg"))
	{
		team = TEAM_BLUE;
	}
#ifdef MISSIONPACK
...
==================
*/
/*void TeamplayInfoMessage(gentity_t *ent)				//Too:
{
	char		entry[1024];
	char		string[8192];
	int			stringlength;
	...
	//for (i = 0, cnt = 0; i < g_maxclients.integer && cnt < TEAM_MAXOVERLAY; i++)
	for (i = 0, k = 0; i < g_maxclients.integer && k < cnt; i++)
	{
		player = g_entities + clients[i];								//Too: "+ i" => "+ clients[i]" a little lazzy code here, imho !;-)
