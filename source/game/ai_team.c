...
int BotValidTeamLeader(bot_state_t *bs)
{
	...
	if ((n = ClientFromName(bs->teamleader)) == -1)
	{
		bs->teamleader[0] = 0;
		return qfalse;
	}
...
int BotSortTeamMatesByBaseTravelTime(bot_state_t *bs, int *teammates, int maxteammates)
{
	...
	if (gametype == GT_CTF //|| gametype == GT_1FCTF
		|| gametype == GT_INVASION || gametype == GT_DESTROY)
	{
...
void BotCTFOrders_BothFlagsAtBase(bot_state_t *bs)
{
	...
	//passive strategy
	else if (!(bs->ctfstrategy & CTFS_AGRESSIVE))
	{
		//different orders based on the number of team mates
		switch (numteammates)
		{
...
/*
======================
Too: Inv_BotDestroyOrders
======================
*/

static void Inv_AttackBase(bot_state_t *bs, int Num)
{
	char name[MAX_NETNAME];

	ClientName(Num, name, sizeof(name));
	BotAI_BotInitialChat(bs, "cmd_attackenemybase", name, NULL);
	BotSayTeamOrder(bs, Num);
	BotSayVoiceTeamOrder(bs, Num, VOICECHAT_OFFENSE);
}

static void Inv_BotDestroyOrders(bot_state_t *bs)
{
	int teammates[MAX_CLIENTS];
	int numteammates, i;
	char buf[MAX_INFO_STRING];
	...
	assert(BotTeam(bs) == level.MarineTeam);
	...
	switch (numteammates)
	{
		...
		case 3:
		{
			BotCreateGroup(bs, teammates, 3);		//a group of 3
			Inv_AttackBase(bs, teammates[0]);
			break;
		}
		case 4:
		...
		default:
		{
			if (numteammates <= 10)
			{
				int n = numteammates / 3, size;

				for (i = 0; i < n; i++)
				{
					if (i == n - 1)
						size = numteammates - i * 3;
					else
						size = 3;

					BotCreateGroup(bs, teammates + i * 3, size);	//groups of 3
					Inv_AttackBase(bs, teammates[i * 3]);
				}
			}
			break;
		}
		...
		default:
		{
			if (numteammates <= 10)
			{
				...
				for (i = 0; i < n; i++)
				{
					...
					BotCreateGroup(bs, teammates + i * 3, 3);	//groups of 3
				}
			}
			break;
		}
				...
				if (!notleader[i])
				{
					...
					if (BotSameTeam(bs, i))
					{
						...
						if (!BotSetLastOrderedTask(bs))
						{
							if (gametype == GT_INVASION)
							{
								int Team = Inv_BotTeam(bs);

								if (Team == level.MarineTeam)
									BotVoiceChat_GetFlag(bs, i, SAY_TELL);
								//else if (Team == level.AlienTeam)
								//	BotVoiceChat_FollowMe(bs, i, SAY_TELL);
							}
							else // go on defense by default
...
void BotTeamAI(bot_state_t *bs)
{
	...
	if (!BotValidTeamLeader(bs))
	{
		if (!FindHumanTeamLeader(bs))
		{
			if (!bs->askteamleader_time && !bs->becometeamleader_time)
			{
				...
				{
					bs->askteamleader_time = FloatTime() + 2 + random() * 5;
				}
				else
				{
					bs->becometeamleader_time = FloatTime() + 5 + random() * 5;
				}
			}
			if (bs->askteamleader_time && bs->askteamleader_time < FloatTime())
			{
				...
				bs->becometeamleader_time = FloatTime() + 3 + random() * 5;
			}
	...
	switch (CurrentGameType)
	{
		case GT_DESTROY:
		{
			...
			{
				Inv_BotDestroyOrders(bs);
				//give orders again after x seconds
				bs->teamgiveorders_time = FloatTime() + 45;
			}
			break;
		}
		case GT_TEAM:
		{
			...
			if (bs->teamgiveorders_time && bs->teamgiveorders_time < FloatTime() - 5)
			{
				BotTeamOrders(bs);
				//give orders again after x seconds
				bs->teamgiveorders_time = FloatTime() + 60;
			}
			break;
		}
		case GT_INVASION:
