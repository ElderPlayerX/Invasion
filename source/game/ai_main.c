...
void BotReportStatus(bot_state_t *bs)
{
	...
	switch (bs->ltgtype)
	{
		...
		{
			ClientName(bs->teamgoal.entitynum, goalname, sizeof(goalname));
			BotAI_Print(PRT_MESSAGE, "%-20s%s%s: tracking prey %s\n", netname, leader, flagstatus, goalname);
			break;
		}
		case LTG_CAMP:
		case LTG_CAMPORDER:
...
void BotSetInfoConfigString(bot_state_t *bs)
{
	...
	switch (bs->ltgtype)
	{
		...
		{
			ClientName(bs->teamgoal.entitynum, goalname, sizeof(goalname));
			Com_sprintf(action, sizeof(action), "tracking prey %s", goalname);
			break;
		}
		case LTG_CAMP:
		case LTG_CAMPORDER:
...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3], int time)
{
	...
	if (bi->actionflags & ACTION_ATTACK2) ucmd->buttons |= BUTTON_ATTACK2;
	if (bi->actionflags & ACTION_TALK) ucmd->buttons |= BUTTON_TALK;
	if (bi->actionflags & ACTION_GESTURE) ucmd->buttons |= BUTTON_GESTURE;
	if (bi->actionflags & ACTION_USE) ucmd->buttons |= BUTTON_USE_HOLDABLE;
	if (bi->actionflags & ACTION_WALK) ucmd->buttons |= BUTTON_WALKING;
	if (bi->actionflags & ACTION_AFFIRMATIVE) ucmd->buttons |= BUTTON_AFFIRMATIVE;
	if (bi->actionflags & ACTION_NEGATIVE) ucmd->buttons |= BUTTON_NEGATIVE;	//Too: not more used
...
//		trap_Cvar_Set("bot_report", "0");
	...
	for (i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (botstates[i]->botthink_residual >= thinktime)
		{
			...
			if (g_entities[i].client->pers.connected == CON_CONNECTED)
			{
				gclient_t *cl = level.clients + botstates[i]->client;

				BotAI(i, (float) thinktime / 1000);

				if (cl->pers.WaitForSelection			//Too: set selection after BotAI() call, because the gender (and other init) is set from BotAI()
					&& botstates[i]->setupcount <= 0)
				{
					--cl->pers.WaitForSelection;

					if (cl->pers.WaitForSelection <= 1)
					{
						char *s;

						cl->pers.WaitForSelection = 1;

						if (cl->sess.sessionTeam == level.MarineTeam)
						{
							int weapon = random() * e_Selection_MaxWeapon,
								armor = random() * e_Selection_MaxArmor;

							//weapon = e_Selection_FlameThrower;
							//armor = e_Selection_Magnetic;

							s = va("InvMarineSelected %i %i", weapon, armor);
						}
						else
						{
							int race = random() * e_Selection_MaxRace;
							if (race == e_Selection_Trooper)
								race = random() * e_Selection_MaxRace;	//Too: low chance to choose this poor Trooper

							s = va("InvAlienSelected %i", race);
						}

						trap_EA_Command(botstates[i]->client, s);
					}
				}
			}
		}
