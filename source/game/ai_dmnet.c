...
int BotNearbyGoal(bot_state_t *bs, int tfl, bot_goal_t *ltg, float range)
{
	...
	//
	if (Inv_GetAlienRace(bs) == e_Selection_Trooper)
...
int BotGetItemLongTermGoal(bot_state_t *bs, int tfl, bot_goal_t *goal)
{
	...
	{
		if (bs->ltgtype != LTG_DEFENDKEYAREA && bs->ltgtype != 0)
		{
			BotChooseWeapon(bs);
			bs->ltg_time = 0;
		}
...
int BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal)
{
	...
	//if the bot tracks someone
	if (bs->ltgtype == LTG_TRACKPREY && !retreat)
	{
		gclient_t *cl;
		...
		if (bs->teamgoal.entitynum >= 0 && bs->teamgoal.entitynum <= MAX_CLIENTS)
		{
			cl = level.clients + bs->teamgoal.entitynum;

			if (cl->pers.connected != CON_CONNECTED
				|| cl->ps.persistant[STAT_HEALTH] <= 0
				|| BotSameTeam(bs, bs->teamgoal.entitynum))
			{
				Inv_BotClearLGT(bs);
			}
	...
	if (gametype == GT_CTF || gametype == GT_INVASION)
	{
		...
		if (bs->ltgtype == LTG_GETFLAG)
		{
			...
			switch (BotTeam(bs))
			{
				...
				default: Inv_BotClearLGT(bs); return qfalse;
			}
		...
		if (bs->ltgtype == LTG_RUSHBASE && bs->rushbaseaway_time < FloatTime())
		{
			...
			if (!BotCTFCarryingFlag(bs)) Inv_BotClearLGT(bs);
			//quit rushing after 2 minutes
			if (bs->teamgoal_time < FloatTime()) Inv_BotClearLGT(bs);
			...
			if (trap_BotTouchingGoal(bs->origin, goal))
			{
				...
				{
					trap_BotResetAvoidReach(bs->ms);
					bs->rushbaseaway_time = FloatTime() + 5;//Too: + 10 * random();
		...
		if (bs->ltgtype == LTG_RETURNFLAG)
		{
			...
			//if touching the flag
			if (trap_BotTouchingGoal(bs->origin, goal)) Inv_BotClearLGT(bs);
			//stop after 3 minutes
			if (bs->teamgoal_time < FloatTime())
			{
				Inv_BotClearLGT(bs);
...
int AINode_Battle_Fight(bot_state_t *bs)
{
	...
	{
		if (bs->enemydeath_time < FloatTime() - 0.5f)		//Too: 1.0f
		{
	...
	else
	{
		if (EntityIsDead(&entinfo))
		{
			if (bs->enemy >= MAX_CLIENTS)
				bs->enemydeath_time = FloatTime() - 5.0f;
			else
	...
	{
		if (BotWantsToChase(bs) || bs->enemy >= MAX_CLIENTS)
		{
...
int AINode_Battle_Retreat(bot_state_t *bs)
{
	...
	if (gametype == GT_DESTROY && bs->enemy >= MAX_CLIENTS)
	{
		AIEnter_Seek_LTG(bs, "battle retreat: fighting egg");
		return qfalse;
	}
