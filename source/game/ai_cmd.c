...
void BotPrintTeamGoal(bot_state_t *bs)
{
	...
	switch (bs->ltgtype)
	{
		...
		case LTG_TRACKPREY:
		{
			BotAI_Print(PRT_MESSAGE, "%s: I'm tracking my prey for %1.0f secs\n", netname, t);
			break;
		}
		default:
		{
...
void BotMatch_StartTeamLeaderShip(bot_state_t *bs, bot_match_t *match)
{
	...
	strncpy(cl->sess.TeamLeader, bs->teamleader, sizeof(bs->teamleader));
}
...
void BotMatch_WhatAreYouDoing(bot_state_t *bs, bot_match_t *match)
{
	...
	//if not addressed to this bot
	//if (!BotAddressedToBot(bs, match)) return;		//Too: put this again after Tests !!!
	//
	switch (bs->ltgtype)
	{
		case LTG_TEAMHELP:
