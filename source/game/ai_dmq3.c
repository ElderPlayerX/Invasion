...
void BotSetUserInfo(bot_state_t *bs, char *key, char *value)
{
	...
	ClientUserinfoChanged(bs->client, qfalse);
}
...
{
	if (gametype != GT_CTF && gametype != GT_INVASION) return CTF_FLAG_NONE;
...
{
	gclient_t *cl = level.clients + bs->client;
...
==================
*/
team_t Inv_BotTeam(bot_state_t *bs)
{
	gclient_t *cl = level.clients + bs->client;
...
/*
==================
Too: Inv_GetAlienRace
==================
*/
int Inv_GetAlienRace(bot_state_t *bs)
{
	if (Inv_BotTeam(bs) == level.AlienTeam)
	{
		gclient_t *cl = level.clients + bs->client;

		return cl->pers.AlienRace;
	}
...
/*
==================
Too: BotEnemyFlagCarrier
==================
*/
int Inv_BotEnemyFlagCarrier(bot_state_t *bs)
{
	int i;
...
/*
==================
Too: Inv_FindClosestEnemy
==================
*/
int Inv_FindClosestEnemy(bot_state_t *bs)
{
	int i, d, AreaNum, Closest = -1, Dist = 0, Team;
	...
	for (i = 0; i < maxclients; ++i)
	{
		...
		if (cl->sess.sessionTeam == TEAM_SPECTATOR || !strlen(cl->pers.netname))
			continue;
		...
		d = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, AreaNum, TFL_DEFAULT);
	...
	//BotAI_Print(PRT_WARNING, "found enemy %d for Bot %d\n", Closest, bs->client);
...
qboolean EntityIsDead(aas_entityinfo_t *entinfo)
{
	...
	if (entinfo->number >= MAX_CLIENTS && gametype == GT_DESTROY)
	{
		gentity_t *Current = g_entities + entinfo->number;

		if (Current->health <= 0)
			return qtrue;

	}
	else if (entinfo->number >= 0 && entinfo->number < MAX_CLIENTS)
	{
		//retrieve the current client state
		BotAI_GetClientState(entinfo->number, &ps);
		if (ps.pm_type != PM_NORMAL || (ps.pm_flags & PMF_FOLLOW))
			return qtrue;
	}
	return qfalse;
}
...
void BotSetTeamStatus(bot_state_t *bs)
{
	...
	switch (bs->ltgtype)
	{
		...
		case LTG_TEAMACCOMPANY:
			...
			if (((gametype == GT_CTF || gametype == GT_INVASION || gametype == GT_1FCTF) && EntityCarriesFlag(&entinfo))
...
int BotSetLastOrderedTask(bot_state_t *bs)
{
	...
	if (!BotSameTeam(bs, bs->lastgoal_decisionmaker))
	{
		bs->lastgoal_ltgtype = 0;
	}
	else if (bs->lastgoal_ltgtype)
	{
...
==================
*/
void Inv_BotClearLGT(bot_state_t *bs)
{
	bs->ltgtype = 0;
...
/*
==================
Too: BotDestroySeekGoals
==================
*/

void BotDestroySeekGoals(bot_state_t *bs)
{
	if (Inv_BotTeam(bs) == level.MarineTeam)
	{
		BotSetLastOrderedTask(bs);

		if (bs->ltgtype == LTG_ATTACKENEMYBASE)
		{
			gentity_t *Current = g_entities + bs->teamgoal.entitynum;

			if (Q_stricmp(Current->classname, "team_CTF_alienegg") || !bs->teamgoal.areanum)
			{
				memset(&bs->teamgoal, 0, sizeof(bs->teamgoal));
				Inv_GetNearestDestroyGoal(bs);
			}
		...
		if ((bs->ltgtype == LTG_ATTACKENEMYBASE
			...
			|| bs->ltgtype == LTG_PATROL) && bs->ordered)
		{
			if (BotSameTeam(bs, bs->decisionmaker))// && bs->client != bs->decisionmaker)
				return;
		}

		if (bs->ltgtype != LTG_ATTACKENEMYBASE || bs->teamgoal_time < FloatTime())
		{
			memset(&bs->teamgoal, 0, sizeof(bs->teamgoal));
			Inv_GetNearestDestroyGoal(bs);
			...
			//set the time the bot will stop
			bs->teamgoal_time = FloatTime() + 15;//TEAM_ATTACKENEMYBASE_TIME;
			//get an alternative route goal towards the enemy base
			BotGetAlternateRouteGoal(bs, BotOppositeTeam(bs));
			BotSetTeamStatus(bs);
		}
	...
	if (Inv_BotTeam(bs) == level.AlienTeam)
	{
		...
		if (bs->ltgtype != LTG_TRACKPREY || bs->owndecision_time < FloatTime())
		{
			int c = Inv_FindClosestEnemy(bs);
			...
			if (c != -1)
			{
				...
				bs->decisionmaker = bs->client;
				bs->ordered = qfalse;
				bs->ltgtype = LTG_TRACKPREY;
				bs->teamgoal.entitynum = c;
				bs->teamgoal_time = bs->owndecision_time = FloatTime() + TRACKPREY_TIME;
				BotSetTeamStatus(bs);
...
void BotCTFSeekGoals(bot_state_t *bs)
{
	...
	if (gametype == GT_INVASION)
	{
		if (BotTeam(bs) == TEAM_RED)
		{
			if (BotTeamFlagCarrier(bs) == -1)
				bs->blueflagstatus = 0;
			else
				bs->blueflagstatus = 1;

			flagstatus = bs->blueflagstatus;// + bs->redflagstatus * 2;
		}
		else
		{
			if (Inv_BotEnemyFlagCarrier(bs) == -1)
				bs->blueflagstatus = 0;
			else
				bs->blueflagstatus = 1;
		...
		if (bs->owndecision_time < FloatTime())
		{
					...
					bs->teamgoal.number == ctf_blueflag.number)))
			{
						...
						(bs->ltgtype != LTG_TEAMACCOMPANY || bs->teammate != c))
				{
					...
					BotSetTeamStatus(bs);
					bs->owndecision_time = FloatTime() + 3;
				}
			}
		}
		return;
	...
	else if (flagstatus == 2)
	{
		if (Inv_BotTeam(bs) == level.AlienTeam)
		{
			...
			if (c != -1)
			{
				if (bs->ltgtype != LTG_TRACKPREY || c != bs->teamgoal.entitynum)
				{
					//BotAI_Print(PRT_WARNING, "Track Prey %d for Bot %d\n", c, bs->client);
		...
		if (bs->owndecision_time < FloatTime())
		{
				...
				bs->ltgtype != LTG_GETITEM)
			{
				...
				bs->ordered = qfalse;
				//
				if (random() < 0.5 && gametype != GT_INVASION)
				{
	...
	if (gametype == GT_INVASION)
	{
		if (Inv_BotTeam(bs) == level.MarineTeam)
		{
			...
			if (bs->ltgtype != LTG_GETFLAG || bs->teamgoal_time < FloatTime())
			{
				BotRefuseOrder(bs);
				bs->decisionmaker = bs->client;
				bs->ordered = qfalse;
	...
	if (gametype == GT_INVASION && level.Period == e_Period_Playing)
	{
		assert(0);
	}
...
/*
==================
Too: Inv_BotDestroyRetreatGoals
==================
*/
void Inv_BotDestroyRetreatGoals(bot_state_t *bs)
{
	//if not already rushing to the base
	/*if (bs->ltgtype != LTG_RUSHBASE)
	{
...
#endif
		else if (gametype == GT_DESTROY)
		{
			Inv_BotDestroyRetreatGoals(bs);
		}
	}
	else
	{
...
char *ClientName(int client, char *name, int size)
{
	...
	strncpy(name, cl->pers.netname, size-1);
	name[size-1] = '\0';
	Q_CleanStr(name);
	return name;
}
...
int ClientFromName(char *name)
{
	...
	char NetName[MAX_NETNAME], ClientName[MAX_NETNAME];

	strcpy(ClientName, name);				//Too: unify the name, coz it's always done like this everywhere else
	Inv_UnifyName(ClientName);
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++)
	{
		...
		//Q_stricmp(Info_ValueForKey(buf, "n"), name);	//Too: Q3 Bug about number of spaces in name

		strcpy(NetName, cl->pers.netname);
		Inv_UnifyName(NetName);

		//if (!Q_stricmp(cl->pers.netname, name))
		if (!Q_stricmp(NetName, ClientName))
			return i;
	}
	return -1;
}
...
void BotChooseWeapon(bot_state_t *bs)
{
	...
	else
	{
		...
		if (newweaponnum == WP_MACHINEGUN)		//Too: bipass Machingun selection if new weapon available (it should not happen, but...:))
		{
			if (bs->inventory[INVENTORY_INVCHAINGUN] && bs->inventory[INVENTORY_CHAINBULLETS] > 40)
				newweaponnum = WP_INVCHAINGUN;
			else if (bs->inventory[INVENTORY_FLAMETHROWER] > 0 && bs->inventory[INVENTORY_NAPALM] > 20)
				newweaponnum = WP_FLAMETHROWER;
				...
				newweaponnum = WP_SHOTGUN;
			else if (bs->inventory[INVENTORY_BFG10K])
				newweaponnum = WP_BFG;
		}

		if (Inv_GetAlienRace(bs) == e_Selection_Rad)
			newweaponnum = WP_LIGHTNING;

		if (Inv_BotTeam(bs) == level.MarineTeam
			&& bs->enemy >= 0 && bs->enemy < maxclients)
		{
			gentity_t *ent = g_entities + bs->enemy;

			if (ent->inuse && ent->client && ent->health > 0)
			{
				if (ent->client->pers.AlienRace == e_Selection_Bloomy)
				{
					vec3_t forward;
					float dist, MinDist = Square(40);

					VectorSubtract(ent->r.currentOrigin, bs->origin, forward);
					dist = VectorLengthSquared(forward);
		...
		if (newweaponnum <= WP_NONE || newweaponnum >= WP_NUM_WEAPONS)
			newweaponnum = WP_GAUNTLET;
...
void BotSetupForMovement(bot_state_t *bs)
{
	...
	//
	//if (bs->walker > 0.5) initmove.or_moveflags |= MFL_WALK;	//Too: no walker in invasion !
	//
		...
		if (offence)
		{
			if (!(bs->teamtaskpreference & TEAMTP_ATTACKER))
			{
				...
				else if (g_spSkill.integer <= 3)
				{
					...
					{
						//
						if ((gametype != GT_CTF || gametype != GT_INVASION || (bs->redflagstatus == 0 && bs->blueflagstatus == 0)) &&
...
void BotUpdateInventory(bot_state_t *bs)
{
	...
	bs->inventory[INVENTORY_INVCHAINGUN] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_INVCHAINGUN)) != 0;
	...
	bs->inventory[INVENTORY_FLAMETHROWER] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_FLAMETHROWER)) != 0;
	...
	bs->inventory[INVENTORY_CHAINBULLETS] = bs->cur_ps.ammo[WP_INVCHAINGUN];
	...
	bs->inventory[INVENTORY_NAPALM] = bs->cur_ps.ammo[WP_FLAMETHROWER];
	bs->inventory[INVENTORY_LIGHTNINGAMMO] = bs->cur_ps.ammo[WP_LIGHTNING];
	//Too: ugly patch
	if (bs->cur_ps.ammo[WP_LIGHTNING] < 10 && Inv_GetAlienRace(bs) == e_Selection_Rad)
		bs->inventory[INVENTORY_LIGHTNINGAMMO] = 10;
...
float BotAggression(bot_state_t *bs)
{
	...
	if (bs->inventory[ENEMY_HEIGHT] > 200) return MinAggressivity;
	//if the bot is very low on health
	if (bs->inventory[INVENTORY_HEALTH] < 60) return MinAggressivity;
	//if the bot is low on health
	if (bs->inventory[INVENTORY_HEALTH] < 80)
	{
		//if the bot has insufficient armor
		if (bs->inventory[INVENTORY_ARMOR] < 40) return MinAggressivity;
	}
	...
	//if the bot can use the flamethrower
	if (bs->inventory[INVENTORY_FLAMETHROWER] > 0 &&
			bs->inventory[INVENTORY_NAPALM] > 40) return 85;
...
int BotWantsToRetreat(bot_state_t *bs)
{
	...
	if (gametype == GT_CTF || gametype == GT_INVASION)
	{
		...
		if (Inv_BotTeam(bs) == level.AlienTeam)
			return qfalse;
		else if (Inv_BotTeam(bs) == level.MarineTeam)
		{
			int flagstatus;
	...
	else if (gametype == GT_DESTROY)
	{
		...
		{
			gentity_t *Current = g_entities + bs->enemy;
			vec3_t Delta;

			if (bs->enemy < 0 || bs->enemy >= MAX_CLIENTS)
				return qfalse;

			VectorSubtract(Current->r.currentOrigin, bs->eye, Delta);
			if ((bs->weaponnum == WP_FLAMETHROWER
					&& VectorLengthSquared(Delta) > Square(500))
				|| VectorLengthSquared(Delta) > Square(1000))
			{
				return qtrue;
			}
...
int BotWantsToCamp(bot_state_t *bs)
{
	...
	if (gametype == GT_INVASION || gametype == GT_DESTROY)
		return qfalse;
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl)
{
	...
	float attack_dist = IDEAL_ATTACKDIST, attack_range = 40;
	vec3_t forward, backward, sideward, hordir, up = {0, 0, 1};
	aas_entityinfo_t entinfo;
	bot_moveresult_t moveresult;
	bot_goal_t goal;
	int Race = Inv_GetAlienRace(bs);
	qboolean EasyKill = qfalse;
	...
	if (bs->attackcrouch_time < FloatTime() - 1)
	{
		...
		if (random() < jumper && Race != e_Selection_Xenomorph)
		{
			movetype = MOVE_JUMP;
		}
		//wait at least one second before crouching again
		else if (Race != e_Selection_Bloomy && bs->attackcrouch_time < FloatTime() - 1 && random() < croucher)
		{
	...
	if (bs->cur_ps.weapon == WP_GAUNTLET)
	{
		if (Race == e_Selection_Xenomorph)
		{
			attack_dist = 20;		//Too: try to allow bots to use gauntlet
			attack_range = 20;
		}
		else
		{
	...
	else
	{
		...
		else if (Race == e_Selection_Bloomy || Race == e_Selection_Rad)
		{
			attack_dist = 30;
			attack_range = 30;
		}
	...
	if (attack_skill <= 0.4
		|| EasyKill
		|| (Race == e_Selection_Bloomy && entinfo.weapon != WP_GAUNTLET)
		|| (Race == e_Selection_Xenomorph
			&& (!EntityIsShooting(&entinfo)
				|| bs->lasthealth <= bs->inventory[INVENTORY_HEALTH])))
	{
		if (!EasyKill || random() > 0.2f)
		{
...
int BotSameTeam(bot_state_t *bs, int entnum)
{
	...
	if (gametype >= GT_TEAM)
	{
		...
		gclient_t *BotCl = level.clients + bs->client;
		...
		if (cl->sess.sessionTeam == BotCl->sess.sessionTeam)
			return qtrue;
	}
	return qfalse;
}
...
{
	int i, c, healthdecrease;
	float f, alertness, easyfragger, vis;
	float squaredist, cursquaredist;
	aas_entityinfo_t entinfo, curenemyinfo;
	vec3_t dir, angles;

	//Too: not very clean to do like that, but it works ... ;)
	c = Inv_BotEnemyFlagCarrier(bs);
	if (c != -1)
	{
		vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, c);
	...
	if (gametype == GT_DESTROY && Inv_BotTeam(bs) == level.MarineTeam
		&& (curenemy < 0 || curenemy >= MAX_CLIENTS))
	{
		bsp_trace_t trace;
		gentity_t *Current;
		...
		for (i = 0; i < 2; ++i)
		{
			if (bs->ltgtype == LTG_ATTACKENEMYBASE)
			{
				if (!i)
				{
					if (curenemy >= MAX_CLIENTS && g_entities[curenemy].health > 0)
					{
						Current = g_entities + curenemy;
					}
					else
					{
						Current = g_entities + bs->teamgoal.entitynum;
						if (Current->health <= 0)
						{
							i = 1;
							Current = Inv_GetClosestEgg(bs);
						}
					}
				}
				else
			...
			{
				if (curenemy >= MAX_CLIENTS && !i && g_entities[curenemy].health > 0)
				{
					Current = g_entities + curenemy;
				}
				else
			...
			if (Current != NULL)
			{
				...
				VectorAdd(Current->r.mins, Current->r.maxs, Origin);
				VectorMA(Current->r.currentOrigin, 0.5f, Origin, Origin);

				if (bs->weaponnum == WP_FLAMETHROWER
					|| bs->weaponnum == WP_SHOTGUN)
				{
					if (VectorLengthSquared(Delta) > Square(400))
						continue;
				}

				BotAI_Trace(&trace, bs->eye, NULL, NULL, Origin, bs->client, CONTENTS_SOLID);
				if (trace.fraction >= 1 || trace.ent == Current->s.number)
				{
					if (Current->s.number == curenemy)
						continue;
						//return qfalse;

					bs->enemy = Current->s.number;
					bs->enemysight_time = FloatTime() - 0.5f;
					bs->enemysuicide = qfalse;
					bs->enemydeath_time = 0;
					bs->enemyvisible_time = FloatTime();
					return qtrue;
...
int BotTeamFlagCarrierVisible(bot_state_t *bs)
{
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++)
	{
		...
		if (vis <= 0)
			return -1;	//Too: why continue if we already found the flag carrier ???
			//continue;
		//
		return i;
	}
...
void BotAimAtEnemy(bot_state_t *bs)
{
	...
	{
		wi.proj.gravity = -g_gravity.value;
	}

	//get the weapon specific aim accuracy and or aim skill
	if (wi.number == WP_MACHINEGUN || wi.number == WP_INVCHAINGUN)
	{
	...
	else if (wi.number == WP_PLASMAGUN || wi.number == WP_FLAMETHROWER)
	{
	...
	if (wi.proj.gravity)
	{
		dist = sqrt(Square(dir[0]) + Square(dir[1]));
		dir[2] -= 0.30f * wi.proj.gravity * dist / wi.speed;	//Too: it should be -0.5f ...
	}
...
void BotCheckAttack(bot_state_t *bs)
{
...
#endif
		if (gametype == GT_DESTROY)
		{
			gentity_t *Current = g_entities + attackentity;
	...
	{
		firethrottle += (1 - firethrottle) * 0.5f;		//Too:

		if (random() > firethrottle && Race != e_Selection_Xenomorph)
		{
			firethrottle *= 0.5f;
	...
	else if (Race == e_Selection_Rad)
	{
		if (Dist > LIGHTNING_RANGE + 20)
			return;
	}
	...
	{
		if (gametype == GT_DESTROY && Race == -1)
		{
			gentity_t *Current = g_entities + bsptrace.ent;
	...
	if (trace.ent != attackentity || attackentity >= MAX_CLIENTS)
	{
		...
		{
			if (gametype == GT_DESTROY)
				BotAI_Trace(&trace, start, NULL, NULL, end, bs->entitynum, MASK_SHOT);

			if (trace.fraction * 1000 < wi.proj.radius
				&& (trace.ent != attackentity || attackentity >= MAX_CLIENTS))
			{
	...
	{
		if (Dist < 100)
		 	trap_EA_Action(bs->client, ACTION_ATTACK2);
	}
	else if (Race == e_Selection_Bloomy)
	{
		if (Dist < 250 && random() < 0.1f)
			trap_EA_Action(bs->client, ACTION_ATTACK2);
	}
	else if (Race == e_Selection_Trooper)
	{
		if (bs->weaponnum == WP_MACHINEGUN)
		{
			if (Dist < 400 && bs->inventory[INVENTORY_GRENADES] > 0)
		 		trap_EA_Action(bs->client, ACTION_ATTACK2);
		}
	}
			...
			if (gametype == GT_CTF || gametype == GT_INVASION)
			{
				switch (state->eventParm)
				{
					...
					case GTS_BLUE_TAKEN:
						...
						bs->flagstatuschanged = qtrue;
						break; //see BotMatch_CTF
					case GTS_DROPPED_EGG:
						bs->blueflagstatus = 0;
						bs->flagstatuschanged = qtrue;
						//ctf_blueflag = Inv_GetFlagItem("Blue", qtrue);
						break;
				}
			}
#ifdef MISSIONPACK
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime)
{
	...
	if (bs->setupcount > 0)
	{
		...
		if (gender[0] == 'm')
		{
			...
			cl->sess.Gender = GENDER_MALE;
		}
		else if (gender[0] == 'f')
		{
			trap_BotSetChatGender(bs->cs, CHAT_GENDERFEMALE);
			cl->sess.Gender = GENDER_FEMALE;
		}
		else
		{
			trap_BotSetChatGender(bs->cs, CHAT_GENDERLESS);
			cl->sess.Gender = GENDER_NEUTER;
		}

		//set the chat name
		ClientName(bs->client, name, sizeof(name));

		if (strcmp(cl->sess.TeamLeader, "0123"))
			strncpy(bs->teamleader, cl->sess.TeamLeader, sizeof(bs->teamleader) - 1);
		bs->teamleader[sizeof(bs->teamleader) - 1] = 0;			//Too: fast antibug solution; no time to check that
...
*/
//Too:
bot_goal_t Inv_GetFlagItem(char *Flag, qboolean GetDropped)
{
	//ToCheck: I've got a weird bug here => The dropped flag is yet in the world after the map_restart
	// It seems to not happen in original Q3; but with my mod in CTF, the bug is here too !

	int i = -1, j = 0;
	bot_goal_t goal, goodgoal;
	char *s;
	qboolean Found = qfalse, Loop = qfalse;

	memset(&goodgoal, 0, sizeof(goodgoal));
	...
	do
	{
		...
		do
		{
			i = trap_BotGetLevelItemGoal(i, s, &goal);

			if (i < 0 && !j)
			{
				//BotAI_Print(PRT_WARNING, "CTF without %s Flag (%s)\n", Flag, s);
			}
			else if (GetDropped)
			{
				if (i >= 0 && ((goal.flags & GFL_DROPPED) || !j))
				{
					if (goal.flags & GFL_DROPPED)
					{
						...
						if (entinfo.modelindex2)		// real dropped egg
						{
							goodgoal = goal;
							Found = qtrue;
						}
					}
					else if (Found == qfalse)
						Found = -1;
					++j;
				}
			}
		...
		if (!Q_stricmp(s, "Blue Flag") && Found != qtrue)
		{
			s = "Alien Egg";
			Loop = qtrue;
		}
		else if (!Q_stricmp(s, "Red Flag") && Found != qtrue)
		{
			s = "Teleporter";
			Loop = qtrue;
		}
	...
	if (Found == qfalse)
		BotAI_Print(PRT_WARNING, "Map without %s Flag\n", Flag);
	else if (Found == -1 && !GetDropped)
		BotAI_Print(PRT_WARNING, "Map with dropped %s Flag\n", Flag);
