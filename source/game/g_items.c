...
void Add_Ammo (gentity_t *ent, int weapon, int count)
{
	...
	if (ent->client->ps.ammo[weapon] > 999)//200)
	{
		ent->client->ps.ammo[weapon] = 999;//200;
	}
}
...
//Too: used when player drop his gun, so he doesn't touch it immediatly
void Touch_Item_Dummy(gentity_t *ent, gentity_t *other, trace_t *trace)
{
	//Too: avoid warnings...
	ent = ent;
	other = other;
...
void Touch_Item_On(gentity_t *ed)
{
	ed->touch = Touch_Item;
	ed->think = Touch_Item_On;
	if (g_gametype.integer == GT_CTF)
		ed->nextthink = level.time + 30 * 1000;
	else
		ed->nextthink = level.time + 60 * 60 * 1000;		// Neither free this entity
	ed->s.eFlags &= ~EF_NODRAW;
...
gentity_t *LaunchItem(gitem_t *item, vec3_t origin, vec3_t velocity, qboolean DontFree)
{
	...
	if (g_gametype.integer >= GT_CTF && item->giType == IT_TEAM)
	{
		...
		{
			dropped->nextthink = 0;							//Too: don't bring back dropped flag
		}
		else
		{
	...
	else if (DontFree)
	{
		...
		dropped->touch = Touch_Item_Dummy;
		dropped->think = Touch_Item_On;
...
gentity_t *Drop_Item(gentity_t *ent, gitem_t *item, float angle, qboolean DontFree)
{
	...
	return LaunchItem(item, ent->s.pos.trBase, velocity, DontFree);
}
...
void FinishSpawningItem(gentity_t *ent)
{
	...
	if (g_gametype.integer == GT_DESTROY
		&& !Q_stricmp(ent->classname, "team_CTF_alienegg"))
	{
		ent->touch = NULL;
