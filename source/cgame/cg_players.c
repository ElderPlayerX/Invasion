...
static qboolean CG_FindClientModelFile(char *filename, int length, clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *base, const char *ext)
{
	...
	if (ci->team == TEAM_BLUE || ci->BaseTeam == TEAM_BLUE)
		team = "default";
	else
		team = "red";
...
===================
*/
static void CG_LoadClientInfo(clientInfo_t *ci, char *DefaultModel)
{
	...
	if (!ci->modelName[0])
		strcpy(ci->modelName, DefaultModel);
	...
	// sounds
	dir = ci->modelName;
	fallback = (cgs.gametype >= GT_TEAM) ? DEFAULT_TEAM_MODEL : DefaultModel;
...
static qboolean CG_ScanForExistingClientInfo(clientInfo_t *ci)
{
	...
	if (!Q_stricmp(ci->modelName, DEFAULT_MODEL) || !ci->modelName[0])
	{
		if (cgs.media.MaleMarineModel->infoValid)
		{
			CG_CopyClientInfoModel(cgs.media.MaleMarineModel, ci);
			return qtrue;
		}
	}

	for (i = 0; i < e_Selection_MaxRace + e_Selection_MaxArmor * 2; ++i)
	{
		if (i < e_Selection_MaxRace)
		{
			match = cgs.media.AlienModel + i;
		}
		else if (i < e_Selection_MaxRace + e_Selection_MaxArmor)
		{
			match = cgs.media.MaleMarineModel + i - e_Selection_MaxRace;
		}
		else
		{
			match = cgs.media.FemaleMarineModel + i - (e_Selection_MaxRace + e_Selection_MaxArmor);
		}
...
======================
*/
static void CG_SetDeferredClientInfo(clientInfo_t *ci, const char *configstring) //Too:
{
	int		i;
	clientInfo_t	*match;
...
//			 Q_stricmp(ci->headSkinName, match->headSkinName) ||
		...
		CG_LoadClientInfo(ci, DEFAULT_MODEL);//(char *) configstring);
		return;
	}
	...
	if (cgs.gametype >= GT_TEAM)
	{
		...
		CG_LoadClientInfo(ci, DEFAULT_MODEL);
		return;
	}
...
======================
*/
void CG_NewClientInfo(int clientNum, const char	*configstring)
{
	clientInfo_t *ci;
	...
	v = Info_ValueForKey(configstring, "cs");
	n = atoi(v);

	assert(ci->cs != n);
	if (n < ci->cs)			//Too: we already have more recent one
	{
		CG_Printf("Got old Config String for client %d\n", clientNum);
		return;
	}
	...
	{		//Too:
		// Base Team
		v = Info_ValueForKey(configstring, "b");
		newInfo.BaseTeam = atoi(v);

		// player Class
		v = Info_ValueForKey(configstring, "ic");
		newInfo.Class = atoi(v);
	...
	if (!CG_ScanForExistingClientInfo(&newInfo))
	{
		...
		/* //Too: never defering load in Invasion
		// if we are defering loads, just have it pick the first valid
		if (forceDefer || (cg_deferPlayers.integer && !cg_buildScript.integer && !cg.loading))
		{
			// keep whatever they had if it won't violate team skins
			CG_SetDeferredClientInfo(&newInfo, configstring);
		...
		if (forceDefer)
		{
			...
			CG_SetDeferredClientInfo(&newInfo, configstring);
			// if we are low on memory, leave them with this model
			CG_Printf("Memory is low.  Using deferred model '%s' for Player '%s'\n",
							newInfo.modelName, newInfo.name);
			newInfo.deferred = qfalse;
		}
		else
		{
			CG_LoadClientInfo(&newInfo, DEFAULT_MODEL);
		}
	}
...
void CG_InvCacheAllModels(void)
{
	...
	for (i = 0; i < e_Selection_MaxRace + e_Selection_MaxArmor * 2; ++i)
	{
		...
		{
			m = 0;
			ci = cgs.media.AlienModel + i;
			strcpy(ci->modelName, AlienModel[i]);
			strcpy(ci->skinName, "default");//"blue");
			strcpy(ci->headModelName, AlienModel[i]);
			strcpy(ci->headSkinName, "default");//"blue");
			ci->team = TEAM_BLUE;
			ci->Class = e_Selection_Trooper + i;
		}
		else if (i < e_Selection_MaxRace + e_Selection_MaxArmor)
		{
			int n = i - e_Selection_MaxRace;
			m = 1;
			ci = cgs.media.MaleMarineModel + n;
			strcpy(ci->modelName, MaleMarineModel[n]);
			strcpy(ci->skinName, "red");
			strcpy(ci->headModelName, MaleMarineModel[n]);
			strcpy(ci->headSkinName, "red");
			ci->team = TEAM_RED;
			ci->Class = e_Selection_Kevlar + n;
		}
		else
		{
			int n = i - (e_Selection_MaxRace + e_Selection_MaxArmor);
			m = 2;
			ci = cgs.media.FemaleMarineModel + n;
			strcpy(ci->modelName, FemaleMarineModel[n]);
			strcpy(ci->skinName, "red");
			strcpy(ci->headModelName, FemaleMarineModel[n]);
			strcpy(ci->headSkinName, "red");
			ci->team = TEAM_RED;
			ci->Class = e_Selection_Kevlar + n;
		}
		...
		if (mem < 4000000 || m == 2)
		{
			...
			if (m == 2)
			{
				CG_CopyClientInfoModel(cgs.media.MaleMarineModel + i - (e_Selection_MaxRace + e_Selection_MaxArmor), ci);
			}
			else if (i == e_Selection_MaxRace)
			{
				CG_CopyClientInfoModel(cgs.media.AlienModel + e_Selection_Trooper, ci);
			}
			else if (PrevCI != NULL)
						...
						int *torsoOld, int *torso, float *torsoBackLerp)
{
	...
	else
	{
		...
		if (Speed)
		{
			if ((cent->currentState.legsAnim & ~ANIM_TOGGLEBIT) == LEGS_BACK
				|| (cent->currentState.legsAnim & ~ANIM_TOGGLEBIT) == LEGS_RUN)
			{
				LegSpeed = Speed / 320.0f;												//Too:
			}
			else if ((cent->currentState.legsAnim & ~ANIM_TOGGLEBIT) == LEGS_BACKCR
				|| (cent->currentState.legsAnim & ~ANIM_TOGGLEBIT) == LEGS_WALKCR)
			{
				LegSpeed = Speed / 120.0f;												//Too:
				if (LegSpeed > 2)
					LegSpeed = 2;
				else if (LegSpeed < 0.66f)
					LegSpeed = 0.66f;
			}
			else
			{
		...
		LegSpeed = cent->currentState.origin2[0] / (10.0f * 320);
	...
	{
		CG_RunLerpFrame(ci, &cent->pe.legs, LEGS_TURN, LegSpeed);
	}
	else
	{
		CG_RunLerpFrame(ci, &cent->pe.legs, cent->currentState.legsAnim, LegSpeed);
	}
...
===============
*/
static void CG_TrailItem(centity_t *cent, qhandle_t hModel, refEntity_t *Torso, refEntity_t *Head, int Dec)
{
	refEntity_t		ent;
	...
	memcpy(ent.axis, Torso->axis, sizeof(ent.axis));
	ent.renderfx = Torso->renderfx;

	Dec = -(Dec + 16);
	VectorMA(Head->origin, Dec, Torso->axis[0], ent.origin);
	VectorMA(ent.origin, -16, Torso->axis[2], ent.origin);
...
===============
*/
static void CG_PlayerPowerups(centity_t *cent, refEntity_t *torso, refEntity_t *Head)
{
	int		powerups;
	clientInfo_t	*ci;
	int clientNum;
	...
	// Special can give a dlight
	if (powerups & (1 << PW_SPECIAL))
	{
		if (ci->team == cgs.InvasionInfo.MarineTeam)
		{
			int Armor = ci->Class & e_Class_MarineArmorMask;

			if (Armor == e_Selection_Magnetic)
				trap_R_AddLightToScene(cent->lerpOrigin, 220 + (rand()&31), 0.4f, 1.0f, 0.2f);
		}
	}
	...
	if (powerups & (1 << PW_REDFLAG))
	{
		if (ci->newAnims)
			...
			CG_TrailItem(cent, cgs.media.redFlagModel, torso, Head, cgs.RedFlagDec);
	...
	if (powerups & (1 << PW_BLUEFLAG))
	{
		if (ci->newAnims)
			...
			CG_TrailItem(cent, cgs.media.blueFlagModel, torso, Head, cgs.BlueFlagDec);
			//CG_TrailItem(cent, cgs.media.blueFlagModel);
		trap_R_AddLightToScene(cent->lerpOrigin, 200 + (rand()&31), 0.2f, 1.0f, 0.2f);//, 1.0);
	}
	...
	if (powerups & (1 << PW_NEUTRALFLAG))
	{
		...
		{
			CG_TrailItem(cent, cgs.media.neutralFlagModel, torso, Head, 8);
		}
...
/*
===============
Too: Inv_NightVisionShader
===============
*/
qboolean Inv_NightVisionShader(refEntity_t *ent, entityState_t *state, int team)
{
	qboolean Vision = qfalse;
	int n = 0;
	...
	if (cg.NightVision >= 2 && !cg.Zoom)
	{
		...
		else if (cg.snap->ps.persistant[PERS_TEAM] == cgs.InvasionInfo.MarineTeam)
		{
			Vision = qtrue;
			if (cg.NightVision == 2
				&& state->number == cg.crosshairClientNum && cg.crosshairClientTime)
			{
				n = 2;
			}
		}
		else if (team != cg.snap->ps.persistant[PERS_TEAM])
		{
			Vision = qtrue;
			n = 1;
		}
	...
	if (Vision)
	{
		ent->customShader = cgs.media.NightVisionCloak[n];
		trap_R_AddRefEntityToScene(ent);
	}
...
{
	if (Inv_NightVisionShader(ent, state, team))
		return;
	...
	else
	{
		...
		if (state->powerups & (1 << PW_SPECIAL))
		{
			ent->customShader = cgs.media.MagneticShader;
			trap_R_AddRefEntityToScene(ent);
		}
...
/*
===============
Too: Inv_GetInfoForDead
===============
*/
clientInfo_t *Inv_GetInfoForDead(int Generic)
{
	int i, Team, Class = Generic & 127;
	clientInfo_t *ci = NULL;

	if (!(Generic & 128))
	{
		Team = cgs.InvasionInfo.MarineTeam;

		for (i = 0; i < e_Selection_MaxArmor; ++i)
		{
			if (!cgs.media.MaleMarineModel[i].infoValid)
				continue;

			if (cgs.media.MaleMarineModel[i].Class == Class)
			{
				ci = cgs.media.MaleMarineModel + i;
				break;
			}
		}
	}
	...
	{
		Team = cgs.InvasionInfo.AlienTeam;

		for (i = 0; i < e_Selection_MaxRace; ++i)
		{
			if (!cgs.media.AlienModel[i].infoValid)
				continue;

			if (cgs.media.AlienModel[i].Class == Class)
			{
				ci = cgs.media.AlienModel + i;
				break;
			}
	...
	for (i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		if (Team == cgs.InvasionInfo.MarineTeam)
		{
			if ((ci->Class & e_Class_MarineArmorMask) == Class)
				return ci;
		}
		else if (Team == cgs.InvasionInfo.AlienTeam)
		{
			if ((ci->Class & e_Class_AlienRaceMask) == Class)
				return ci;
		}
	}
...
void CG_Player(centity_t *cent)
{
	...
	if ((cent->currentState.eFlags & EF_DEAD) && cent->currentState.number >= MAX_CLIENTS)
	{
...
/*#ifdef MISSIONPACK
	...
	/*if (cent->pe.GunBackTime) //ToCheck: l'arme tourne comme il faut, mais le point de ref est pas bien placé (d'où décalage par rapport à la main dans Q3)
	{
		float coeff;
		vec3_t angles, axis[3], TorsoAxis[3];

		angles[ROLL] = angles[YAW] = angles[PITCH] = 0;
		...
		if (coeff >= 1)
			...
			angles[PITCH] -= 80 * sin(coeff * 3.141);

		memcpy(TorsoAxis, torso.axis, sizeof(TorsoAxis));
		AnglesToAxis(angles, axis);

		MatrixMultiply(axis, TorsoAxis, torso.axis);
	}*/

	CG_AddPlayerWeapon(&torso, NULL, cent, ci->team);

	// add powerups floating behind the player
	CG_PlayerPowerups(cent, &torso, &head);
	...
	if (ci->team == cgs.InvasionInfo.AlienTeam)
	{
		...
		if (Race == e_Selection_Rad)
			trap_R_AddLightToScene(cent->lerpOrigin, 128 + sin(cg.time/100) * 32.0, 0.1f, 1.0f, 0.1f);
	}
}
