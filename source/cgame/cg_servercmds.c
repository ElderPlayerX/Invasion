...
void CG_ParseServerinfo(void)
{
	...
	cgs.InvasionInfo.gametype = cgs.gametype;
...
/*
==================
Too: CG_ParseGameTimeLeft
==================
*/
static void CG_ParseGameTimeLeft(void)
{
	const char *info;

	info = CG_ConfigString(CS_GAMETIMELEFT);

	cg.GameTimeLeft = atoi(info);
	cg.GameTimeLeftCount = -1;

	if (cg.GameTimeLeft > cg.time)
		cgs.InvasionInfo.Period = e_Period_Playing;
}


/*
==================
Too: CG_ParseInvasionInfo
==================
*/
static void CG_ParseInvasionInfo(void)
{
	const char *info;

	info = CG_ConfigString(CS_INVASIONINFO);

	cgs.InvasionInfo.gametype = cgs.gametype;
	cgs.InvasionInfo.MarineTeam = info[0] - '0';
	cgs.InvasionInfo.AlienTeam = info[1] - '0';
	cgs.InvasionInfo.Period = info[2] - '0';
}


/*
==================
Too: CG_ParseInvGoalPos
==================
*/
static void CG_ParseInvGoalPos(void)
{
	const char *info;

	info = CG_ConfigString(CS_INVGOALPOS);

	assert(strlen(info) < sizeof(cg.RadarGoal) - 1);

	strcpy(cg.RadarGoal, info);
}


/*
=================
CG_ParseInvClientStat
//Too: for all client (except receiver): clientNum life armor
=================
*/
static void CG_ParseInvClientStat(void)
{
	int i, j;
	int client;
	int NbPlayer = 0;
	...
	int NbChar = (Team == cgs.InvasionInfo.AlienTeam) ? 3 : 4;

	while (s[0])
	{
		int n = Inv_NumFromString(s, NbChar);
		s += NbChar;
		++NbPlayer;

		client = n & ((1 << es_InvCS_Num) - 1);

		if (NbChar == 3)
		{
			cgs.clientinfo[client].health = n >> es_InvCS_Num;
			cgs.clientinfo[client].armor = 0;
		}
		else
		{
			cgs.clientinfo[client].health = (n >> es_InvCS_Num) & ((1 << es_InvCS_Health) - 1);
			cgs.clientinfo[client].armor = (n >> (es_InvCS_Num + es_InvCS_Health)) & ((1 << es_InvCS_Armor) - 1);
		}
	}

	for (i = j = 0; i < MAX_CLIENTS; ++i, ++ci)
	{
		if (i == cg.clientNum || ci->infoValid == qfalse)
			continue;

		if (ci->team == TEAM_SPECTATOR ||
			(ci->team != Team && Team == cgs.InvasionInfo.MarineTeam))
			continue;

		sortedTeamPlayers[j] = i;
		++j;
	}

	numSortedTeamPlayers = j;
}
...
=================
CG_ParseInvRadar
//Too: radar position; no client info, just dots
=================
*/

void CG_ParseInvRadar(void)
{
	assert(strlen(CG_Argv(1)) < sizeof(cg.Radar) - 1);

	strcpy(cg.Radar, CG_Argv(1));
}
...
/*#ifdef MISSIONPACK
	...
	cg.GameTimeLeft = atoi(CG_ConfigString(CS_GAMETIMELEFT));
	CG_ParseInvasionInfo();
}
...
static void CG_ConfigStringModified(void)
{
	...
	else if (num == CS_GAMETIMELEFT)
	{
		CG_ParseGameTimeLeft();
	}
	else if (num == CS_INVASIONINFO)
	{
		CG_ParseInvasionInfo();
	}
	else if (num == CS_INVGOALPOS)
	{
		CG_ParseInvGoalPos();
	}
	...
	else if (num >= CS_PLAYERS && num < CS_PLAYERS+MAX_CLIENTS)
	{
		...
		for (i = 3; i < sizeof(buf); ++i)
		{
			...
			if (buf[i] == 0)
			{
				CG_Printf("Error: can't parse player info %d\n", num - CS_PLAYERS);
				return;
			}
		}

		CG_NewClientInfo(num - CS_PLAYERS, buf + i);
...
static void CG_AddToTeamChat(const char *str)
{
	...
	if (cg.time - cgs.teamChatMsgTimes[cgs.teamChatPos % chatHeight] < cg_teamChatTime.integer)
		CG_Printf("%s\n", cgs.teamChatMsgs[cgs.teamChatPos % chatHeight]);
...
static void CG_MapRestart(void)
{
	...
	if (cg.warmup == 0 /* && cgs.gametype == GT_TOURNAMENT */)
	{
		...
		CG_CenterPrint("FIGHT!", 120, GIANTCHAR_WIDTH*1.0f);
	}
#ifdef MISSIONPACK
...
static void CG_ServerCommand(void)
{
	...
	if (!Q_strncmp("Rd", cmd, 2) && cmd[2] && !cmd[3])
	{
		cg.RadiationLevel += (cmd[2] - '0') * 4;
	...
	if (!strcmp(cmd, "IARS"))
	{
		cg.RespawnTime = cg.time + atoi(CG_Argv(1)) - 500;
		return;
	}
	...
	if (!strcmp(cmd, "rp"))
	{
		CG_RightPrint(CG_Argv(1), SCREEN_HEIGHT - ICON_SIZE - BIGCHAR_WIDTH, BIGCHAR_WIDTH);
		return;
	}
	...
	if (!Q_strncmp("Inv_ui", cmd, 6) && !cg.demoPlayback)
	{
		char message[1024];

		trap_Args(message, sizeof(message));

		trap_SendConsoleCommand(va("%s %s ", cmd, message));
