...
//Too:
static void CG_SelectMainGun_f(void)
{
	int i, weapons = cg.snap->ps.stats[STAT_WEAPONS];
	...
	for (i = WP_GAUNTLET; i < MAX_WEAPONS; ++i)
	{
		...
		if (weapons & (1 << i))
		{
			trap_SendConsoleCommand(va("weapon %d", i));
			break;
		}
	}
}
...
//Too:
static void CG_NightVision_f(void)
{
	...
	trap_S_StartLocalSound(cgs.media.sfx_VisionSwap, CHAN_AUTO);

	cg.VisionTime = cg.time;
...
//Too:
static void CG_InGameHelp_f(void)
{
...
{
	if (cgs.gametype == GT_CTF /*|| cgs.gametype == GT_1FCTF*/ || cgs.gametype == GT_INVASION)
	{
...
{
	{ "Inv_SMG", CG_SelectMainGun_f },
	{ "Inv_IGH", CG_InGameHelp_f },
	{ "Inv_NV", CG_NightVision_f },
...
void CG_InitConsoleCommands(void)
{
	...
	trap_AddCommand ("Inv_DropMainGun");
}
