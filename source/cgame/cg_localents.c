...
void CG_BloodTrail(localEntity_t *le)
{
	...
	vec3_t	newOrigin;
	localEntity_t	*blood;
	qhandle_t Trail = cgs.media.bloodTrailShader;
	vec4_t color;

	if (le->leMarkType == LEMT_RADBLOOD)
	{
		Trail = cgs.media.RadBloodTrail;
		Vector4Copy(le->color, color);
	}
	else if (le->leMarkType == LEMT_ACID)
	{
		Trail = cgs.media.AcidTrail;
		Vector4Copy(le->color, color);
	}
	else
	{
...
void CG_FragmentBounceMark(localEntity_t *le, trace_t *trace)
{
	...
	{
		radius = 16 + (rand()&31);
		CG_ImpactMark(cgs.media.RadBloodMark, trace->endpos, trace->plane.normal, random()*360,
			le->color, qtrue, radius, qfalse);
	}
	else if (le->leMarkType == LEMT_ACID)
	{
		radius = 16 + (rand()&31);
		CG_ImpactMark(cgs.media.AcidMark, trace->endpos, trace->plane.normal, random()*360,
			le->color, qtrue, radius, qfalse);
	}
	else if (le->leMarkType == LEMT_BLOOD)
	{
		...
		CG_ImpactMark(cgs.media.bloodMarkShader, trace->endpos, trace->plane.normal, random()*360,
			colorWhite, qtrue, radius, qfalse);
	}
...
static void CG_AddSpriteExplosion(localEntity_t *le)
{
	...
	re.shaderRGBA[0] = le->color[0] * 255.9f;
	re.shaderRGBA[1] = le->color[1] * 255.9f;
	re.shaderRGBA[2] = le->color[2] * 255.9f;
	f = le->color[3] * 255.9f * c;
	if (f >= 256)
		re.shaderRGBA[3] = 255;
	else
		re.shaderRGBA[3] = f;
	...
	re.reType = RT_SPRITE;
	re.radius = le->radius * (1.0 - c) + le->radius * (30.0f / 42);
...
void CG_AddInvulnerabilityJuiced(localEntity_t *le)
{
	...
	{
		le->endTime = 0;
		CG_GibPlayer(le->refEntity.origin, -1);
	}
	else
	{
		trap_R_AddRefEntityToScene(&le->refEntity);
...
===================
*/
void Inv_Tracer(localEntity_t *le)
{
	vec3_t		forward;
	polyVert_t	verts[4];
	float		len, begin, end, coeff;
	vec3_t		start, finish;
	float 		*source, *dest;
	int i, j;
	vec3_t Up = { 0, 0, 1 }, Right = { 1, 0, 0 };

	source = le->refEntity.origin;
	dest = le->refEntity.oldorigin;
	...
	coeff = (float) (le->endTime - cg.time) / (le->endTime - le->startTime);
	...
	if (fabs(DotProduct(forward, Up)) < 0.90f)
	{
	...
	verts[0].st[0] = 0;
	verts[0].st[1] = 1;
	verts[0].modulate[3] = (byte) (255.9f * coeff);

	verts[1].st[0] = 1;
	verts[1].st[1] = 0;
	verts[1].modulate[3] = (byte) (255.9f * coeff);

	verts[2].st[0] = 1;
	verts[2].st[1] = 1;
	verts[2].modulate[3] = 0;//(byte) (0.25f * 255.9f * coeff);

	verts[3].st[0] = 0;
	verts[3].st[1] = 0;
	verts[3].modulate[3] = 0;//(byte) (0.25f * 255.9f * coeff);
