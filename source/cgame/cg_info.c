...
{
	int		n, WidthI = 640, WidthP = 640;
	int		x, y, sI = 32, dI = 48, sP = 64, dP = 78;
	int		modI = WidthI / dI;

	if (loadingItemIconCount * dI > 2 * WidthI)
	{
		dI = 2 * WidthI / loadingItemIconCount;
		modI = WidthI / dI;
		sI = dI * 2 / 3;
	}

	if (loadingPlayerIconCount * dP > WidthP)
	{
		dP = WidthP / loadingPlayerIconCount;
		sP = dP * 64 / 78;
	}
	...
	for (n = 0; n < loadingPlayerIconCount; n++)
	{
		x = 8 + n * dP;
		y = 324;
		CG_DrawPic(x, y, sP, sP, loadingPlayerIcons[n]);
	}
	...
	for (n = 0; n < loadingItemIconCount; n++)
	{
		...
		CG_DrawPic(x, y, sI, sI, loadingItemIcons[n]);
	}
}
